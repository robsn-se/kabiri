...
mysqli_insert_id(); //Возвращает значение, сгенерированное для столбца AUTO_INCREMENT последним запросом

is_numeric();  // Определяет, является ли переменная числом или числовой строкой

substr();   // Возвращает часть строки
// Пример: $rest = substr("abcdef", 2, -1);  // returns "cde"

strlen($str); //Возвращает длину заданного string. возвращает int!
//$str = 'abcdef';
//echo strlen($str); // 6

password_verify(); // Проверяет, что пароль соответствует хешу 1-ый параметр пароль пользователя,
// а второй хеш пароля

password_hash($password, $userData[0]["password"] );  // Создает хэш пароля 1-ый параметр пароль пользователя,
// а второй это сам хеш

basename(); //это встроенная функция PHP, которая извлекает имя файла по заданному пути
// Пример: echo "1) ".basename("/etc/sudoers.d", ".d").PHP_EOL;     1) sudoers

move_uploaded_file(); // перемещает загруженный файл в новое местоположение
// Пример ($_FILES["avatar"]["tmp_name"], "../" . $data["avatar"])
// 1-ый параметр откуда, 2-ой куда